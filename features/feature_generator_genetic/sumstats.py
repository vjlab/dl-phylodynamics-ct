...
def find_identical_clusters(tree, threshold=0.0000000001):
    cluster_size = defaultdict(int)

    # Mark all leaves as unvisited
    visited = set()
    ...
    for node in tree.get_leaves():
        ...
        cluster = set([node.name])
        visited.add(node.name)

        for other_node in tree.get_leaves():
            if other_node.name in visited:
                continue

            distance = tree.get_distance(node, other_node)
            if distance <= threshold:
                cluster.add(other_node.name)
                visited.add(other_node.name)

        if len(cluster) > 0:
            cluster_tuple = tuple(sorted(cluster))
            cluster_size[cluster_tuple] = len(cluster_tuple)

    return cluster_size

def distribution_identical_clusters(tre, dimension=10):
    # Find leaf clusters and their frequencies
    node_clusters = find_identical_clusters(tre)

    frequency = Counter(node_clusters.values())

    output_list = [frequency.get(i, 0) for i in range(1, dimension)]

    output_list.append(len(node_clusters)-sum(output_list))

    return output_list


def leaves_nodes_distribution(tre, dimension=30):
    ...
    for node in tre.traverse("levelorder"):
        if not node.is_leaf():
            dist_int.append(node.get_distance(tre))
        else:
            dist_ext.append(node.get_distance(tre))
	
    output_list_ext = [dist_ext.count(i) for i in range(0, dimension+1)]

    output_list_int = [dist_int.count(i) for i in range(0, dimension+1)]

    return output_list_ext+output_list_int
