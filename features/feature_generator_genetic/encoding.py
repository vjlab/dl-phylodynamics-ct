...
def encode_into_summary_statistics(tree_input, sampling_proba):
    ...
    #summaries.extend(sumstats.compute_chain_stats(tree, order=2))
    # summaries.extend(sumstats.compute_chain_stats(tree, order=3))
    # summaries.extend(sumstats.compute_chain_stats(tree, order=5))
    # summaries.extend(sumstats.compute_chain_stats(tree, order=6))
    # summaries.extend(sumstats.compute_chain_stats(tree, order=7))
    # summaries.extend(sumstats.compute_chain_stats(tree, order=8))
    
    # summaries.extend(sumstats.branchdifference(tree))
    # summaries.extend(sumstats.branches_deciles(tree))

    #identical sequence clusters
    summaries.extend(sumstats.distribution_identical_clusters(tree))
    summaries.extend(sumstats.leaves_nodes_distribution(tree))
...
def encode_into_most_recent(tree_input, sampling_proba):
    ...
    def complete_coding(encoding, max_length):
        ...
        add_vect = np.repeat(-1, max_length - len(encoding))
    ...
    def refactor_to_final_shape(result_v, sampling_p, maxl):
        def reshape_coor(max_length):
            ...
            tips_coor = np.insert(tips_coor, 0, max_length + 1)

            int_nodes_coor = np.arange(1, max_length + 1, 2)
            int_nodes_coor = np.insert(int_nodes_coor, 0, max_length+2)
            #int_nodes_coor = np.insert(int_nodes_coor, -1, max_length + 2)

            print(int_nodes_coor)
        ...
        if maxl == 1999:
            result_v.loc[:, 1999] = -1.0
            result_v['2000'] = sampling_p
            result_v['2001'] = sampling_p
            print(result_v)
        else:
            result_v.loc[:, 400] = 0
    ...
    # local copy of input tree
    tree_refactor = tree_input.copy()
    # compute the rescale factor
    rescale_factor = rescale_tree(tree_refactor, target_avg_length=TARGET_AVG_BL)
