	...
	with open(tree_file,'r') as f:
		...
		for fileline in filelines:
			...
			if encoding_method == "SUMSTATS":
				encoded_tree = encode_into_summary_statistics(tree, float(epi_param['sampling probability']))
				#encoded_tree, rescale_factor = encode_into_summary_statistics(tree, epi_param.iloc[row,3])
				...
				encoded_tree, rescale_factor = encode_into_most_recent(tree, epi_param['sampling probability'])
				encoded_tree[len(encoded_tree.columns)] = rescale_factor
			row = row + 1
			#print(encoded_tree)
